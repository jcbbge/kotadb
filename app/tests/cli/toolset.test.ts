...
describe("filterToolsByTier", () => {
	test("filterToolsByTier('core') returns exactly 14 core tools", async () => {
	...
	test("filterToolsByTier('default') returns exactly 16 tools (core + sync)", async () => {
		const { filterToolsByTier } = await import("@mcp/tools.js");
	...
	test("filterToolsByTier('memory') returns exactly 19 tools (core + sync + memory)", async () => {
		const { filterToolsByTier } = await import("@mcp/tools.js");
	...
	test("filterToolsByTier('full') returns exactly 24 tools (all tiers)", async () => {
		const { filterToolsByTier } = await import("@mcp/tools.js");

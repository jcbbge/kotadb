...
function visitNode(
	...
	switch (node.type) {
		...
		case "ExpressionStatement":
			// Handle Factory.register("name", ...) legacy module pattern
			extractFactoryRegister(node, symbols);
			break;
		default:
...
/**
 * Extract Factory.register("name", fn) legacy module pattern.
 * Matches: Factory.register("<name>", function(...) { ... })
 */
function extractFactoryRegister(
	...
	symbols.push({
		...
		signature: `Factory.register("${name}", ...)`,
		documentation: null,
		isExported: true,
		isAsync: false,
	});

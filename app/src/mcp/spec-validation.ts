...
export async function validateImplementationSpec(
	...
	try {
		...
		const repositoryId = await resolveRepositoryIdentifier(spec.repository);
		...
		const fileConflicts = await checkFileConflicts(spec, repositoryId);
...
async function checkFileConflicts(
	...
	if (spec.files_to_create) {
		for (const fileSpec of spec.files_to_create) {
			const fileId = await resolveFilePath(fileSpec.path, repositoryId);
...
function generateApprovalConditions(
	...
	if (spec.migrations && spec.migrations.length > 0) {
		conditions.push(
			"Verify SurrealDB schema changes in app/src/db/surreal/client.ts",
		);
